...
if(!$user['username']) {
    ...
    CURLOPT_POSTFIELDS =>"{\n    \"APIKey\": \"".$config['apikey2']."\",\n    \"NetworkCode\": \"".strtoupper($card_type)."\",\n    \"PricesExchange\": \"$card_amount\",\n    \"NumberCard\": \"$card_code\",\n    \"SeriCard\": \"$card_seri\",\n    \"IsFast\": true,\n    \"UrlCallback\": \"\",  \"RequestId\": \"$request_id\"\n}",
    ...
    $myfile = fopen("log/log_nap.txt", "a");
    $txt = $user['username']."|".date('H:i:s d/m/y', time())."|".$request_id."|".$responseDecode['status']."|".$responseDecode['message']."\n";
    fwrite($myfile, $txt);
    fclose($myfile);
    ...
    else if($responseDecode['status'] == 401){
          die($response);
    }
    else if($responseDecode['status'] == 400){
         die($response);
    }
    else
    {
