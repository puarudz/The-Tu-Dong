...
foreach ($syntax as $key) {
	...
	show(array('status' => 'false', 'code' => '400', 'message' => 'Mật khẩu không được có kí tự lạ!'));
    	...
    	$check = mysqli_query($kun->connect_db(), "SELECT `password` FROM `users` WHERE `password`='".md5($op)."' AND `username`='".$user['username']."' ");
...
if($rp !== $p) {
    ...
    $res = mysqli_query($kun->connect_db(), "UPDATE users SET password = '".md5($p)."' WHERE `username`='".$user['username']."'");
  
sleep(2);
  	show(array('status' => 'true', 'code' => '200', 'message' => 'Đổi mật khẩu thành công!'));  